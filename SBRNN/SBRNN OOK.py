...
class SBRnn(nn.Module):
    def __init__(self, feature, hidden_size, num_layers, M):
        ...
        self.fc = nn.Sequential(nn.Linear(hidden_size * 2, self.out_size),nn.Softmax())
    ...
    def forward(self, x, L):
        ...
        sliding_res = torch.zeros([batch, Seq_length, self.out_size],requires_grad = False)
        jk = torch.zeros(Seq_length)
...
def cal_received_signal(data,samples,Seq_length,omega,num):
    ...
    diff_receive=diff_receive.reshape(batch,-1,samples)
...
if __name__ == "__main__":
    ...
    received_signal = cal_received_signal(label, a, Seq_length, omega, train_num)
    ...
    received_signal = cal_received_signal(label, a, Seq_length, omega, test_num)
